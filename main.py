...
async def guarded_setforums_handler(update, context):
    if not _is_allowed(update):
        await update.message.reply_text(PRIVATE_MSG, parse_mode="HTML")
        return
    await setforums_handler(update, context)
...
def build_main_app() -> Application:
    ...
    app.add_handler(CommandHandler("setforums", guarded_setforums_handler))
